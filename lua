local function catwareShowLoadingNotification()
    local RunService = game:GetService("RunService")
    local guiParent = (gethui and gethui()) or game:GetService("CoreGui")
    local gui = Instance.new("ScreenGui")
    gui.Name = "catwareLoadingNotification"
...
getgenv().catwareModuleCache = getgenv().catwareModuleCache or {}

local function catwareSafeRequire(moduleRef, timeoutSec)
    local cache = getgenv().catwareModuleCache
    ...
    error("[catware] module load failed: " .. key .. " (" .. tostring(lastErr) .. ")")
end

getgenv().catwareRequire = catwareSafeRequire

local function catwarePreloadCoreModules()
    ...
    pcall(function() catwareSafeRequire(modulesFolder:WaitForChild("EnumLibrary", 4)) end)
    pcall(function() catwareSafeRequire(modulesFolder:WaitForChild("Utility", 4)) end)
    local controllers = playerScripts:WaitForChild("Controllers", 8)
    pcall(function() catwareSafeRequire(controllers:WaitForChild("FighterController", 4)) end)
    pcall(function() catwareSafeRequire(controllers:WaitForChild("CameraController", 4)) end)
end

local function catwareHideErrorPromptsStep()
...
local function catwareRunLoadingBootstrap()
    local runService = game:GetService("RunService")
    local dismissLoading = catwareShowLoadingNotification()
    local hideErrors = true
    local hideConn = runService.RenderStepped:Connect(function()
        if hideErrors then catwareHideErrorPromptsStep() end
    end)

    task.wait(0.8)

    pcall(catwarePreloadCoreModules)
...
local function catwarePlainReplace(str, find, replace)
    local startPos = 1
    while true do
...
getgenv().catwareSliderFillState = getgenv().catwareSliderFillState or {}
getgenv().catwareSliderFollowRate = getgenv().catwareSliderFollowRate or 11

getgenv().catwareSetSliderFill = function(fill, targetX, hideBorder, sliderObj, forceSnap)
    if not fill or not fill.Parent then return end
    targetX = math.floor(tonumber(targetX) or 0)
    local stateMap = getgenv().catwareSliderFillState
    if forceSnap then
...
if not getgenv().catwareSliderFillBound then
    getgenv().catwareSliderFillBound = true
    RunService.RenderStepped:Connect(function(dt)
        local stateMap = getgenv().catwareSliderFillState
        if not stateMap then return end
        dt = math.clamp(tonumber(dt) or 0, 1 / 1000, 1 / 15)
        local follow = getgenv().catwareSliderFollowRate or 11
...
local function loadcatwareLibrary()
    local cacheKey = "catwareLibrary_Cached_Patched"
    if getgenv()[cacheKey] then return getgenv()[cacheKey]() end
    local rawSrc
    if isfile and isfile("catwareLibrary_Patched.lua") then
        rawSrc = readfile("catwareLibrary_Patched.lua")
    ...
    if not rawSrc then
        if isfile and isfile("catwareLibrary_Patched.lua") then
            pcall(delfile, "catwareLibrary_Patched.lua")
        end
...
local Slider = {
        ...
        function Slider:Display()
            ...
            getgenv().catwareSetSliderFill(Fill, X, HideBorderRight, Slider);
...
SliderInner.InputBegan:Connect(function(Input)
    if Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame() then
        ...
        while InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
            ...
            getgenv().catwareSetSliderFill(Fill, nX, HideBorderRight, Slider);
...
local MinSlider = CreateSlider(true, UDim2.new(0, 0, 0, 0));
        ...
        function MultiSlider:Display()
            ...
            getgenv().catwareSetSliderFill(MinSlider.Fill, MinX);
            getgenv().catwareSetSliderFill(MaxSlider.Fill, MaxX);
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame() then
                ...
                while InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
                    ...
                    if nValue <= MultiSlider.MaxValue then
                        ...
                        getgenv().catwareSetSliderFill(MinSlider.Fill, nX);
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame() then
                ...
                while InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
                    ...
                    if nValue >= MultiSlider.MinValue then
                        ...
                        getgenv().catwareSetSliderFill(MaxSlider.Fill, nX);
...
return Library]==]
    ...
    local replaces = {
        ["Fill.Size = UDim2.new(0, X, 1, 0);"] = "getgenv().catwareSetSliderFill(Fill, X, HideBorderRight, Slider);",
        ["Fill.Size = UDim2.new(0, nX, 1, 0);"] = "getgenv().catwareSetSliderFill(Fill, nX, HideBorderRight, Slider);",
        ["MinSlider.Fill.Size = UDim2.new(0, MinX, 1, 0);"] = "getgenv().catwareSetSliderFill(MinSlider.Fill, MinX);",
        ["MaxSlider.Fill.Size = UDim2.new(0, MaxX, 1, 0);"] = "getgenv().catwareSetSliderFill(MaxSlider.Fill, MaxX);",
        ["MinSlider.Fill.Size = UDim2.new(0, nX, 1, 0);"] = "getgenv().catwareSetSliderFill(MinSlider.Fill, nX);",
        ["MaxSlider.Fill.Size = UDim2.new(0, nX, 1, 0);"] = "getgenv().catwareSetSliderFill(MaxSlider.Fill, nX);",
        ["ScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Global;"] = "ScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Global;\nScreenGui.DisplayOrder = 2147483646;\nScreenGui.IgnoreGuiInset = true;",
        ["AccentColor = Color3.fromRGB(200, 149, 108)"] = "AccentColor = Color3.fromRGB(255, 40, 40)",
        ["AccentColor = Color3.fromRGB(200, 149, 106)"] = "AccentColor = Color3.fromRGB(255, 40, 40)",
        ["AccentColor = Color3.fromRGB(200, 149, 107)"] = "AccentColor = Color3.fromRGB(255, 40, 40)",
        ["AccentColor = Color3.fromRGB(0, 110, 55)"] = "AccentColor = Color3.fromRGB(255, 40, 40)",
    }
    for old, new in pairs(replaces) do
        patchedSrc = patchedSrc:gsub(old:gsub("([%(%)%.%%%+%-%*%?%[%]%^%$])", "%%%1"), new)
    end
    if writefile then pcall(writefile, "catwareLibrary_Patched.lua", patchedSrc) end
    ...
    error("[catware] library failed to load")
end

local Library = loadcatwareLibrary()

local INSTANCE_ACCENT = Color3.fromRGB(255, 40, 40)
...
getgenv().catwareApplyAccentTheme = applyInstanceAccentTheme
...
local function applycatwareUiLayering()
    pcall(function()
    ...
    pcall(function()
        local st = getgenv().catwareCosmeticUIState
    ...
    pcall(function()
        local kb = getgenv().catwareKeybindList
...
getgenv().catwareApplyUiLayering = applycatwareUiLayering
...
applycatwareUiLayering()

getgenv().catwareMenuCursor = getgenv().catwareMenuCursor or {}

local function ensurecatwareMenuCursor()
    local mc = getgenv().catwareMenuCursor
...
local function iscatwareMenuOpen()
...
local function shouldSuppressGameplayOverlays()
    return iscatwareMenuOpen()
end

ensurecatwareMenuCursor()

local MENU_CURSOR_HZ = 240
local MENU_CURSOR_BIND = "catwareMenuCursorDraw"
local menuCursorAccum = 0

local function drawcatwareMenuCursor()
    local mc = getgenv().catwareMenuCursor
    if not mc or not mc.cursor or not mc.outline then return end
    local open = iscatwareMenuOpen()
    if not open then
    ...
    local accent = (Library and Library.AccentColor) or Color3.fromRGB(255, 40, 40)
...
pcall(function()
    RunService:BindToRenderStep(MENU_CURSOR_BIND, Enum.RenderPriority.Last.Value + 10, function(dt)
        ...
        drawcatwareMenuCursor()
    end)
end)

getgenv().catwareIsMenuOpen = iscatwareMenuOpen

getgenv().catwareLanguage = getgenv().catwareLanguage or "english"
...
getgenv().catwareTranslateWeapon = function(name)
    if not name or type(name) ~= "string" then return name end
    local lang = getgenv().catwareLanguage or "english"
    if lang == "english" then return name end
    local pack = InstanceWeaponTranslations[lang]
    return (pack and pack[name]) or name
end

getgenv().catwareTranslateLabel = function(text)
    if not text or type(text) ~= "string" then return text end
    local lang = getgenv().catwareLanguage or "english"
    if lang == "english" then return text:lower() end
    local translated = getgenv().catwareTranslateWeapon(text)
    if translated ~= text then return translated:lower() end
    return text:lower()
end

getgenv().catwareGetUiFont = function()
    if getgenv().catwareLanguage == "korean" and getgenv().catwareUIFont then
        return getgenv().catwareUIFont
    end
    if Library and Library.Font then
        return Library.Font
    end
    return getgenv().catwareUIFont or Enum.Font.Roboto
end

getgenv().catwareL = function(key, ...)
    local lang = getgenv().catwareLanguage or "english"
...
local InstanceFontSources = {
    english = { file = "catware_ui_en.otf", fontFile = "catware_ui_en.font", name = "catwareUI_En", url = "https://github.com/nyrus573l/esp-fonts/raw/refs/heads/main/fortnite.otf" },
    korean = { file = "catware_ui_ko.otf", fontFile = "catware_ui_ko.font", name = "catwareUI_Ko", url = "https://github.com/googlefonts/noto-cjk/raw/main/Sans/OTF/Korean/NotoSansCJKkr-Regular.otf" },
    spanish = { file = "catware_ui_es.otf", fontFile = "catware_ui_es.font", name = "catwareUI_Es", url = "https://github.com/nyrus573l/esp-fonts/raw/refs/heads/main/fortnite.otf" },
}

local function loadcatwareLanguageFont(lang)
    lang = lang or getgenv().catwareLanguage or "english"
    ...
    if ok and face then
        getgenv().catwareUIFont = face
        return face
    end
    getgenv().catwareUIFont = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium)
    return getgenv().catwareUIFont
end

getgenv().catwareReloadLanguageFont = function(lang)
    lang = lang or getgenv().catwareLanguage or "english"
    if lang == "english" and Library and Library.Font then
        getgenv().catwareUIFont = Library.Font
        return Library.Font
    end
    return loadcatwareLanguageFont(lang)
end
task.defer(function()
    pcall(function()
        getgenv().catwareReloadLanguageFont(getgenv().catwareLanguage)
    end)
end)

getgenv().catwareDrawingTextPool = getgenv().catwareDrawingTextPool or {}

getgenv().catwareTrackDrawingText = function(obj)
    table.insert(getgenv().catwareDrawingTextPool, obj)
    pcall(function()
        local face = getgenv().catwareUIFont
        if face and obj.FontFace ~= nil then obj.FontFace = face end
    end)
    return obj
end

getgenv().catwareApplyDrawingUIFont = function()
    local face = getgenv().catwareUIFont
    if not face then return end
    for _, obj in ipairs(getgenv().catwareDrawingTextPool) do
        pcall(function()
...
getgenv().catwareApplyUiFont = function(obj, textSize)
    if not obj then return end
    if textSize then obj.TextSize = textSize end
    local font = getgenv().catwareGetUiFont()
    pcall(function()
        if typeof(font) == "Font" then
            obj.FontFace = font
        else
...
getgenv().catwareApplyUIFont = function()
    local face = getgenv().catwareGetUiFont()
    if not face then return end
    getgenv().catwareUIFont = face
    ...
    if getgenv().catwareApplyDrawingUIFont then pcall(getgenv().catwareApplyDrawingUIFont) end
    if getgenv().catwareApplyKbListTheme then pcall(getgenv().catwareApplyKbListTheme) end
end

getgenv().catwareHudFonts = {
    ...
    { n = "custom", draw = 2, face = function() return getgenv().catwareUIFont or Font.fromEnum(Enum.Font.Gotham) end },
}
getgenv().catwareHudFontNames = {}
for i, slot in ipairs(getgenv().catwareHudFonts) do
    getgenv().catwareHudFontNames[i] = slot.n
end
getgenv().catwareHudFontIdx = 1
getgenv().catwareHudDrawFont = 2

getgenv().catwareHudFontIndexByName = function(name)
    for i, slot in ipairs(getgenv().catwareHudFonts) do
...
getgenv().catwareApplyHudFont = function(idx)
    idx = math.clamp(tonumber(idx) or getgenv().catwareHudFontIdx or 1, 1, #getgenv().catwareHudFonts)
    local slot = getgenv().catwareHudFonts[idx]
    getgenv().catwareHudFontIdx = idx
    getgenv().catwareHudDrawFont = slot.draw or 2
    local face
    pcall(function() face = slot.face() end)
    if face then getgenv().catwareEspUiFont = face end
    if _G.ESPObjects then
        for _, box in pairs(_G.ESPObjects) do
            if box and box.text then
                for _, lbl in pairs(box.text) do
    ...
    for _, obj in ipairs(getgenv().catwareDrawingTextPool or {}) do
        pcall(function()
            if obj.Font ~= nil then obj.Font = getgenv().catwareHudDrawFont end
            if face and obj.FontFace ~= nil then obj.FontFace = face end
        end)
    end
end
...
local Utility           = catwareSafeRequire(ReplicatedStorage.Modules.Utility)
local EnumLibrary       = catwareSafeRequire(ReplicatedStorage.Modules.EnumLibrary)
local LocalPlayer       = Players.LocalPlayer
...
getgenv().catwareGetAspectStretch = getAspectStretch
getgenv().catwareWorldToScreen = worldToScreen
getgenv().catwareScreenAnchor = screenAnchor
getgenv().catwareScreenCenter = screenCenter
getgenv().catwareGetUnstretchedCameraCFrame = getUnstretchedCameraCFrame

_G.Features = _G.Features or {}
_G.Features.CrossbowSoundId = "rbxassetid://165946246"
getgenv().catwareCombatLastShotAt = getgenv().catwareCombatLastShotAt or 0

local function markCombatShot()
    getgenv().catwareCombatLastShotAt = tick()
end
getgenv().catwareMarkCombatShot = markCombatShot

getgenv().catwareWorldToScreenEsp = function(worldPos, cam)
...
getgenv().catwareRefreshGunSoundMute = refreshGunSoundMute
...
getgenv().catwareHandleLocalShot = handleLocalShot
...
local fonts = {}
fonts.main = getgenv().catwareUIFont or Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Black)
...
hitSoundsTab:AddToggle("DisableGunSounds", {
    ...
    Callback = function(val)
        ...
        if val then
            if getgenv().catwareRefreshGunSoundMute then
                getgenv().catwareRefreshGunSoundMute()
            end
        else
...
getgenv().catwareUpdViewModelChams = updViewModelEffectLoop

applyChams = function(dt)
    if iscatwareMenuOpen and iscatwareMenuOpen() then
        return
    end
...
local function viewportloop()
    ...
    viewportLoop = game:GetService("RunService").RenderStepped:Connect(function()
        if iscatwareMenuOpen and iscatwareMenuOpen() then return end
...
local modules = {
    enums = catwareSafeRequire(replicatedstorage.Modules.EnumLibrary),
    fighter = catwareSafeRequire(player.PlayerScripts.Controllers.FighterController),
    camcontrol = catwareSafeRequire(player.PlayerScripts.Controllers.CameraController),
    utility = catwareSafeRequire(replicatedstorage.Modules.Utility)
}
...
local _catwareLobbyCache = { t = 0, v = true }
local function lobbyCache()
    local now = os.clock()
    if now - _catwareLobbyCache.t < 0.15 then
        return _catwareLobbyCache.v
    end
    _catwareLobbyCache.t = now
    _catwareLobbyCache.v = not liveMatch()
    return _catwareLobbyCache.v
end

getgenv().catwareIsInActiveMatch = liveMatch
getgenv().catwareIsInLobby = lobbyCache
...
local function getammo()
    ...
    local success, controller = pcall(function()
        return catwareSafeRequire(player.PlayerScripts.Controllers.FighterController)
    end)
    ...
    for _, targetplayer in players:GetPlayers() do
        if targetplayer ~= player and targetplayer.Character then
            ...
            if valid(char) then
                local root = char:FindFirstChild("HumanoidRootPart")
                if root then
                    local wts = getgenv().catwareWorldToScreen or worldToScreen
...
local function stopsync()
    ...
    clrVoidSnap()
    config.voidspam.phase = nil
    if config.orbit.savedpos and localfighter and localfighter.Entity and localfighter.Entity.RootPart and not getgenv().catwareUndergroundEnabled then
...
local function startsync()
    ...
    config.orbit.connection = runsvc.Heartbeat:Connect(function(dt)
        ...
        if not getgenv().catwareUndergroundEnabled then
            if localfighter and localfighter.Entity and localfighter.Entity.RootPart then
                local currentPos = localfighter.Entity.RootPart.CFrame
...
modules.camcontrol.Update = function(...)
    if getgenv().catwareUndergroundEnabled then
        return oldcamupdate(...)
...
local function updatesling()
    ...
    if slingon then
        ...
        if getgenv().catwareSetUnderground then
            getgenv().catwareSetUnderground(true)
        end
        ...
        if not Toggles.AntiAimUnderground.Value then
            if getgenv().catwareSetUnderground then
                getgenv().catwareSetUnderground(false)
            end
        end
        vhState.active = false
    end
...
runsvc.RenderStepped:Connect(function()
    local inLobby = getgenv().catwareIsInLobby and getgenv().catwareIsInLobby()
...
local function mkStatusLbl()
    local label = getgenv().catwareTrackDrawingText(Drawing.new("Text"))
    label.Size = 13
    label.Font = 2
    label.Outline = true
    label.Center = true
...
local function shouldNotifyPlayerHit(plr)
    ...
    local lastShot = getgenv().catwareCombatLastShotAt or 0
...
local function pollHitNotifHealth()
    ...
    for _, plr in ipairs(players:GetPlayers()) do
        ...
        if cur < last - 0.01 then
            ...
            ragePerf.lastHitAtByPlayer[plr] = tick()
            if cur <= 0 then
                local tryKill = getgenv().catwareTryKillSound
                if tryKill then
...
local function bindHitNotifCharacter(plr, char)
    ...
    hitNotifHumConn[plr] = hum.HealthChanged:Connect(function(newHp)
        ...
        if newHp <= 0 and localHit and localHit.plr and localHit.plr ~= player and not config.hitNotifications.enabled then
            local tryKill2 = getgenv().catwareTryKillSound
            if tryKill2 then
...
if not config.profile.regionspoof then
    config.profile.regionspoof = { enabled = false, value = ".gg/banware" }
end
...
regionTab:AddInput("RegionSpoofValue", {
    ...
    Callback = function(val)
        config.profile.regionspoof.value = val or ".gg/banware"
...
v2:AddToggle("cframespf_enabled", {
    ...
    Callback = function(state)
        ...
        if getgenv().catwareConfigLoading then return end
    end
...
v2:AddToggle("cframefly_enabled", {
    ...
    Callback = function(state)
        ...
        if getgenv().catwareConfigLoading then
            return
        end
        if not state then
            cleanupfly()
...
getgenv().catwareCleanupMovement = function()
    cleanupfly()
    _G.keyheldcframe = false
    _G.keyheldcframefly = false
    local char = LP.Character
...
local fighter_controller = catwareSafeRequire(game:GetService("Players").LocalPlayer.PlayerScripts.Controllers.FighterController)
local local_fighter = fighter_controller.LocalFighter
local underground_enabled = false
local camera_controller = catwareSafeRequire(game:GetService("Players").LocalPlayer.PlayerScripts.Controllers.CameraController)
...
local function updantiaim(deltatime)
    ...
    if getgenv().catwareConfigLoading then return end
    if getgenv().catwareConfigLoading == nil then return end
...
getgenv().catwareFlushMovementState = function()
    flushAntiAimMovementState()
    if getgenv().catwareCleanupMovement then
        pcall(getgenv().catwareCleanupMovement)
    else
        _G.keyheldcframe = false
        _G.keyheldcframefly = false
    end
...
getgenv().catwareSyncAfterConfigLoad = function()
    if getgenv().catwareFlushMovementState then
        pcall(getgenv().catwareFlushMovementState)
    end
    ...
    if Toggles and Toggles.AntiAimUnderground then
        ...
        getgenv().catwareUndergroundEnabled = underground_enabled
        if not underground_enabled then
            underground_oldpos = nil
        end
    else
        underground_enabled = false
        getgenv().catwareUndergroundEnabled = false
        underground_oldpos = nil
    end
...
antiaimbox:AddToggle("AntiAimUnderground", {
    ...
    Callback = function(value)
        underground_enabled = value
            getgenv().catwareUndergroundEnabled = value
            if getgenv().catwareConfigLoading then return end
            if not value then
            underground_oldpos = nil
        end
...
getgenv().catwareSetUnderground = function(val)
    underground_enabled = val
    getgenv().catwareUndergroundEnabled = val
    if not val then
        underground_oldpos = nil
    end
end
...
local getAspectStretch = getgenv().catwareGetAspectStretch
local worldToEspScreen = getgenv().catwareWorldToScreenEsp or getgenv().catwareWorldToScreen
local espScreenAnchor = getgenv().catwareScreenAnchor
...
local function trackUtilDrawingText(obj)
    if getgenv().catwareTrackDrawingText then
        return getgenv().catwareTrackDrawingText(obj)
    end
    return obj
end
...
local function makedrawtext(text, color)
    ...
    t.Center = true
    t.Font = getgenv().catwareHudDrawFont or 2
    return t
end
            ...
            if onScreen and utilconfig.enable and isselected(realname) then
                ...
                if distdraw then
                    if utilconfig.distance then
                        ...
                        distdraw.Text = getgenv().catwareL("studs_short", dist)
                        if showImg then
...
getgenv().crosshair = {
    ...
    crosshair_color = Color3.fromRGB(255, 40, 40),
    
    color1 = Color3.fromRGB(255, 40, 40),
...
local WATERMARK_TEXT = "catware"

local textChars = {}
for i = 1, #WATERMARK_TEXT do
    local char = getgenv().catwareTrackDrawingText(Drawing.new("Text"))
    char.Size = 13
    char.Font = 2
    char.Outline = true
...
local ammoLabel = getgenv().catwareTrackDrawingText(Drawing.new("Text"))
ammoLabel.Size = 13
ammoLabel.Font = 2
...
getgenv().catwarePlayKillSound = playKillSound

getgenv().catwareTryKillSound = function(plr, lastHp, newHp)
    if not ksCfg.Enabled then
        return
    end
    if not plr or plr == ksLocalPlayer then
...
getgenv().catwareStopSkyboxUpdater = stopSkyboxUpdater
...
game:GetService("ScriptContext").DescendantRemoving:Connect(function(descendant)
    if descendant == script then
        local stopSkyboxUpdater = getgenv().catwareStopSkyboxUpdater
...
Library:OnUnload(function()
    ...
    getgenv().catwareKeybindList = nil
    Library.Unloaded = true
end)

Library.KeybindFrame.Visible = false

getgenv().catwareKeybindList = getgenv().catwareKeybindList or {}
local kbList = getgenv().catwareKeybindList
local KB_ROW_H = 18
local KB_PAD = 8
local KB_TITLE_H = 28
...
local function kbTextWidth(text, textSize)
    ...
    local ok, sz = pcall(function()
        local fontFace = getgenv().catwareGetUiFont and getgenv().catwareGetUiFont()
...
local function buildKbListUi()
    ...
    getgenv().catwareApplyUiFont(title, 14)
    ...
    for i = 1, KB_MAX_ROWS do
        ...
        lbl.Parent = rowFrame
        getgenv().catwareApplyUiFont(lbl, 12)
...
applyKbListTheme = function()
    ...
    local accent = kbTheme("AccentColor", Color3.fromRGB(255, 40, 40))
    ...
    if kbList.title then
        ...
        getgenv().catwareApplyUiFont(kbList.title, 14)
    end
    ...
    for _, row in ipairs(kbList.rowPool) do
        if row.lbl then
            ...
            getgenv().catwareApplyUiFont(row.lbl, 12)
        end
        if row.frame then
...
getgenv().catwareApplyKbListTheme = function()
    kbThemeDirty = true
    applyKbListTheme()
end
getgenv().catwareRefreshKbList = function(force)
...
do
    local prevSync = getgenv().catwareSyncAfterConfigLoad
    getgenv().catwareSyncAfterConfigLoad = function()
...
local function syncMenuOverlayUi()
    ...
    if open and getgenv().catwareCosmeticUIState and getgenv().catwareCosmeticUIState.applyLiveViewModelUpdate then
        task.defer(function()
            pcall(getgenv().catwareCosmeticUIState.applyLiveViewModelUpdate)
        end)
    end
    if open and getgenv().catwareApplyCosmeticUiTheme then
        pcall(getgenv().catwareApplyCosmeticUiTheme)
    end
end
...
MenuGroup:AddDropdown('catwareLanguage', {
    Text = 'language',
    Values = { 'english', 'korean', 'spanish' },
    Default = getgenv().catwareLanguage or 'english',
    Callback = function(val)
        getgenv().catwareLanguage = val
        if getgenv().catwareReloadLanguageFont then
            getgenv().catwareReloadLanguageFont(val)
        end
        if getgenv().catwareApplyUIFont then
            getgenv().catwareApplyUIFont()
        end
        if getgenv().catwareRefreshLocaleUI then
            pcall(getgenv().catwareRefreshLocaleUI)
        end
    end,
})
...
local function catwareHasTargetTexture(inst)
...
local function recordTargetTexture(inst)
    if not inst or targetTextureOrig[inst] or not catwareHasTargetTexture(inst) then return end
...
local function degradeTextureInstance(inst)
    if not inst or not catwareHasTargetTexture(inst) then return end
...
local function processTargetTextureInstance(inst)
    ...
    if catwareHasTargetTexture(inst) then degradeTextureInstance(inst) end
    for _, child in ipairs(inst:GetDescendants()) do
        if catwareHasTargetTexture(child) then degradeTextureInstance(child) end
    end
end
...
getgenv().catwareUpdSmoothTextures = updSmoothTextures
getgenv().catwareUpdDarkTextures = updDarkTextures
getgenv().catwareUpdTransparentTextures = updTransparentTextures
...
texturesBox:AddToggle('smooth_textures', {
    ...
    Callback = function(val)
        ...
        local fn = getgenv().catwareUpdSmoothTextures
        if fn then fn() end
    end
})
...
texturesBox:AddToggle('dark_textures', {
    ...
    Callback = function(val)
        ...
        local fn = getgenv().catwareUpdDarkTextures
        if fn then fn() end
    end
})
...
local transparentToggle = texturesBox:AddToggle('transparent_textures', {
    ...
    Callback = function(val)
        ...
        local fn = getgenv().catwareUpdTransparentTextures
        if fn then fn() end
    end
})
...
function ThemeManager:ThemeUpdate()
    ...
    if getgenv().catwareApplyAccentTheme then
        pcall(getgenv().catwareApplyAccentTheme)
    end
    if applyTargetHudTheme then
        applyTargetHudTheme()
    end
    if getgenv().catwareApplyCosmeticUiTheme then
        pcall(getgenv().catwareApplyCosmeticUiTheme)
    end
    if getgenv().catwareKeybindList and getgenv().catwareKeybindList.inner then
        if getgenv().catwareApplyKbListTheme then
            pcall(getgenv().catwareApplyKbListTheme)
        end
    end
end
...
SaveManager:SetFolder('catware/rivals')
SaveManager:BuildConfigSection(Tabs['UI Settings'])

getgenv().catwareConfigLoading = true
applyInstanceAccentTheme()
